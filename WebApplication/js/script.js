...
{ var matricola= document.getElementById("matricolaRegText")
  var nome=  document.getElementById("nomeRegText");
var cognome= document.getElementById("cognomeRegText");
var password = document.getElementById("passwordRegText");
var email=document.getElementById("emailRegText");
var squadra=document.getElementById("squadraRegText");
var facolta= document.getElementById("sceltaFacolta");
var reg=document.getElementById("alertreg");
if(matricola.value!=""&&nome.value!=""&&cognome.value!=""&&password.value!=""&&email.value!=""&&squadra.value!=""){
var xml= connectA("metodo=registrazione&matricola="+matricola.value+ "&nome=" + nome.value + "&cognome=" + cognome.value +  "&password=" + password.value+ "&username="+ email.value + "&facolta="+ facolta.value+ "&squadra=" + squadra.value);
    ...
    else {reg.style.visibility="visible";window.scrollTo(0, 0);}
    ...
    if (admin.checked){
       ...
       var xml= connectA("metodo=login&utente=" + ut.value + "&password=" + pass.value);
        ...
        if (xml.readyState == 4 && xml.status == 200) {
            ...
            if(t[0].status=="true")
            window.location=("fantasquadra.html");
            else
                { 
               aler.style.visibility = "visible";
            }
    ...
    xml.onreadystatechange = function() {
        if (xml.readyState == 4 && xml.status == 200) {
            ...
            for (prop in t) {
                switch (t[prop].ruolo) {
                    case "P":
                        portieri.push(t[prop]);
                        break;
                    case "D":
                        difensori.push(t[prop]);
                        break;
                    case "C":
                        centrocampisti.push(t[prop]);
                        break;
                    case "A":
                        attaccanti.push(t[prop]);
                        break;
                }
            }
...
function pulisciSelectFormazione() {
    ...
    var centrocampistiopt = document.getElementById("optFormazioneCentrocampisti");
    var attaccantiopt = document.getElementById("optFormazioneAttaccanti");
     var portierioptp = document.getElementById("optPanchinaPortieri");
    var difensorioptp = document.getElementById("optPanchinaDifensori");
    var centrocampistioptp = document.getElementById("optPanchinaCentrocampisti");
    var attaccantioptp = document.getElementById("optPanchinaAttaccanti");
    resetSelect(portieriopt);
    resetSelect(difensoriopt);
    resetSelect(centrocampistiopt);
    resetSelect(attaccantiopt);
    resetSelect(portierioptp);
    resetSelect(difensorioptp);
    resetSelect(centrocampistioptp);
    resetSelect(attaccantioptp);
}
...
function riempiFormazioneSelect() {
   setTimeout(riempiFormazione(), 5000);
}
