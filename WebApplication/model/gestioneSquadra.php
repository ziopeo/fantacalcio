<?php
	...
	function insertGiocatoriRosa($utente, $giocatoriJSON){
		...
		foreach ($giocatoriJSON as $selected_option) {
			...
			$conn=getDatabase();
			mysqli_query($conn, $sql) or $sw=false;;
		}
	...
	function setGiocatoriFormazioneGiornata($utente, $titolari, $panchinari, $modulo){
		...
		for ($i=0;$i<count($titolari);$i++)inserisciGiocatoreFormazione($titolari[$i], $idForm, 1);
		for ($i=0;$i<count($panchinari);$i++)inserisciGiocatoreFormazione($panchinari[$i], $idForm, 0);
		return true;
	}
...
SELECT MAX(idFormazione) FROM Formazione, Utente WHERE UTENTE LIKE "'.$utente. '") AND Giocatore.idGiocatore LIKE Schierato.Giocatore) AS sq LEFT JOIN PagellaGiocatore ON(sq.idGiocatore = PagellaGiocatore.giocatore)) as sq1';
	...
	function getFormazioneUtenteModel($utente){
		$id=getIdUltimaFormazioneConsegnata($utente);
		$sql="SELECT giocatore, tipoSchieramento, ruolo, nome, squadra FROM SCHIERATO, GIOCATORE, FORMAZIONE  WHERE SCHIERATO.`Formazione` LIKE FORMAZIONE.IDFORMAZIONE AND GIOCATORE.IDGIOCATORE LIKE SCHIERATO.GIOCATORE AND SCHIERATO.FORMAZIONE='$id' AND  FORMAZIONE.UTENTE LIKE '$utente'";
		$conn=getDatabase();
	...
	function getIdUltimaFormazioneConsegnata($utente){
		$sw=true;
		$sql="SELECT idformazione, MAX(idFORMAZIONE) FROM UTENTe,  FORMAZIONE  WHERE  FORMAZIONE.UTENTE LIKE '$utente'";
		$conn=getDatabase();
		$result= mysqli_query($conn, $sql) or $sw=false;
		$row=mysqli_fetch_assoc($result);
		
		if($sw) return $row['idformazione']; else return $sw;
	}
